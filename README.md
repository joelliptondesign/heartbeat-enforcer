...
Structured, enforceable explanations for AI-generated code changes.
...
Every AI code change must produce a complete, self-contained explanation.

If it doesn’t, the run is considered incomplete.
...
You come back to a change and think:
“Why did this file get modified?”

### With this

You open the record and see exactly what changed and why.
...
Heartbeat Enforcer ensures every AI run produces a structured explanation of what changed and why. In a workflow using this tool, every run must produce one structured event describing the work that was just done.
...
A valid run includes a complete, self-contained explanation. If the record is malformed, incomplete, refers back to outside context, or does not match the expected file coverage, the run is considered incomplete.

## Key Benefits

- Deterministic enforcement of complete records
- Full change coverage when used with `--changed-files`
- Self-contained reasoning in the repository
- No dependence on chat history
- Works with any AI coding tool that can append a JSONL record automatically
...
These explanations are generated by the AI and are not perfect, but they are consistently captured and checked against the actual file changes. This significantly reduces missing or misleading explanations and makes reasoning far more reliable than relying on memory or chat history alone.
    ...
    "operations": [
      {
        ...
        "reason": "Implemented authentication flow as requested"
      },
      {
        "mode": "autonomous",
        "files": ["config.py"],
        "action": "created",
        "reason": "Added configuration file to store auth settings"
      },
      {
        "mode": "autonomous",
        "files": ["requirements.txt"],
        "action": "modified",
        "reason": "Added dependency for authentication library"
      }
    ]
  }
}
...
Without this, those extra changes would be easy to miss, misinterpret, or attribute to the wrong cause.
...
You do not write these records yourself.

Your AI coding tool appends them automatically after each run.

In practice, you set this up once by instructing your AI coding tool to append them as part of every run.
...
- Configure your agent once (recommended):
  ...
  Every run must produce a complete, self-contained heartbeat event.
...
You get consistent, reviewable reasoning without changing how you work.
...
- Merge is blocked if heartbeat validation fails.
- Required file: `telemetry/executor_heartbeat.jsonl`.
- Heartbeat records must be generated by tooling.
- Heartbeat coverage must include all changed files.
- CI logs keep the raw validator JSON and print readable failure reasons.
